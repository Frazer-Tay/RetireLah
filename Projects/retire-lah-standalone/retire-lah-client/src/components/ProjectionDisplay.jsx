...
const ProjectionDisplay = ({ data }) => {
  ...
  const formatCurrency = (value, fractionDigits = 2) => {
    if (value === undefined || value === null || value === Infinity || !isFinite(value)) return <span className="text-red-600">N/A (Goal may be unachievable)</span>;
  ...
  const Card = ({ children, title, icon }) => (
    <div className="bg-brand-card-bg p-6 sm:p-8 rounded-xl shadow-xl ring-1 ring-black ring-opacity-5">
      {title && (
        <h2 className="text-xl sm:text-2xl font-semibold text-brand-primary mb-4 sm:mb-6 flex items-center">
          {icon && React.cloneElement(icon, { className: "h-6 w-6 sm:h-7 sm:w-7 mr-3 text-brand-primary" })}
          {title}
        </h2>
      )}
      {children}
  ...
  const InfoIcon = () => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" /></svg>;
  const ChartIcon = () => <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path d="M2 11a1 1 0 011-1h2a1 1 0 011 1v5a1 1 0 01-1 1H3a1 1 0 01-1-1v-5zM8 7a1 1 0 011-1h2a1 1 0 011 1v9a1 1 0 01-1 1H9a1 1 0 01-1-1V7zM14 4a1 1 0 011-1h2a1 1 0 011 1v12a1 1 0 01-1 1h-2a1 1 0 01-1-1V4z" /></svg>;

  return (
    <div className="mt-8 sm:mt-10 space-y-8 sm:space-y-10">
      <Card title="Your Retirement Snapshot" icon={<ChartIcon />}>
        <p className="text-sm text-brand-light-text mb-6 -mt-2 sm:-mt-4 ml-10">Based on your inputs for retiring at age {assumptions.retirementAge} with {assumptions.lifestyleExpenses}.</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-blue-50 p-6 rounded-lg shadow-inner border border-blue-100">
            <p className="text-sm font-medium text-blue-700 mb-1">Target Nest Egg</p>
            <p className="text-3xl xl:text-4xl font-bold text-brand-secondary">{formatCurrency(targetNestEgg)}</p>
            <p className="text-xs text-blue-600 mt-2">Total amount needed by age {assumptions.retirementAge} to fund your desired lifestyle, based on a {assumptions.safeWithdrawalRate} withdrawal rate.</p>
          </div>
          <div className="bg-green-50 p-6 rounded-lg shadow-inner border border-green-100">
            <p className="text-sm font-medium text-green-700 mb-1">Monthly Investment Needed</p>
            <p className="text-3xl xl:text-4xl font-bold text-brand-secondary">{formatCurrency(monthlyInvestmentNeeded)}</p>
            <p className="text-xs text-green-600 mt-2">Invest this much monthly in {assumptions.instrumentName} to reach your goal, considering estimated returns and fees.</p>
          </div>
        </div>
      </Card>
      
      <Card title="Projection Assumptions" icon={<InfoIcon />}>
      ...
      <Card title="Investment Fee Information" icon={<InfoIcon />}>
        <p className="text-sm text-brand-medium-text mb-4 -mt-2 sm:-mt-4 ml-10">Expense ratios for the available investment instruments. Lower fees mean more of your money stays invested.</p>
        <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
            <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                    <tr>
                        <th scope="col" className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Ticker</th>
                        <th scope="col" className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Full Name</th>
                        <th scope="col" className="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Expense Ratio</th>
                    </tr>
                </thead>
