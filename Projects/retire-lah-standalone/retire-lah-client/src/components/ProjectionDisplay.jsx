    ...
    <div className="mt-8 sm:mt-10 space-y-8 sm:space-y-10">
      <Card title="Your Retirement Snapshot" IconComponent={ChartBarIcon}>
        <p className="text-sm text-brand-light-text mb-6 -mt-3 sm:-mt-4 ml-10">
          Based on your inputs for retiring at age {assumptions.retirementAge} with a future lifestyle cost of SGD {assumptions.futureMonthlyLifestyleCost?.toLocaleString()}/month.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-green-50 p-6 rounded-xl shadow-sm border border-green-200">
            ...
            <p className="text-xs text-green-600 mt-2 leading-relaxed">Invest this much monthly in {assumptions.instrumentName} to reach your goal, considering your initial investment, estimated returns, and fees.</p>
          </div>
        </div>
      </Card>
      ...
      <Card title="Projection Assumptions" IconComponent={InfoIcon}>
        <ul className="list-disc list-inside text-sm text-brand-medium-text space-y-2 pl-1 sm:pl-3">
          ...
          <li>Estimated Gross Annual Return: <span className="font-semibold text-brand-dark-text">{assumptions.grossAnnualReturn}</span> (Net: <span className="font-semibold text-brand-dark-text">{assumptions.netAnnualReturn}</span>)</li>
          <li>Safe Withdrawal Rate (SWR): <span className="font-semibold text-brand-dark-text">{assumptions.safeWithdrawalRate}</span></li>
          <li className="italic text-xs pt-2 text-brand-light-text">Calculations assume monthly investments are made at the start of each month. Figures are estimates and rounded. Past performance is not indicative of future results. Does not account for taxes or other fees beyond ETF ER.</li>
        </ul>
      </Card>
