...
const InputForm = ({ formData, setFormData, onSubmit, isLoading }) => {
  // console.log("InputForm RENDERED. Props:", { isLoading }); // Removed formData from here to avoid excessive logging on every keystroke

  const handleChange = (e) => {
    const { name, value } = e.target;
    const processedValue = (name === "currentAge" || name === "retirementAge") && value !== "" ? Number(value) : value;
    console.log(`InputForm handleChange: name=${name}, value=${value}, processedValue=${processedValue}`);
    setFormData(prev => {
      const newState = { ...prev, [name]: processedValue };
      console.log("InputForm setFormData called. New local formData would be:", JSON.stringify(newState));
      return newState;
    });
  };

  const localFormSubmit = (e) => {
    e.preventDefault(); // Ensure default is prevented here
    console.log("InputForm localFormSubmit triggered. Calling onSubmit prop.");
    onSubmit(e); // Call the handleSubmit passed from App.js
  }

  return (
    // Pass localFormSubmit to the form's onSubmit
    <form onSubmit={localFormSubmit} className="bg-brand-card-bg p-6 sm:p-8 rounded-xl shadow-card-lg space-y-6"> 
      ...
      <div>
        ...
        <select name="lifestyle" id="lifestyle" value={formData.lifestyle} onChange={handleChange} required className="py-2.5 px-3.5 pr-10">
      ...
      {/* The button type="submit" will trigger the form's onSubmit */}
      <button type="submit" disabled={isLoading} className="w-full bg-brand-primary hover:bg-brand-primary-dark text-white font-semibold py-3 px-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 ease-in-out disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center text-base transform active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-primary">
        {isLoading ? ( /* ... loading spinner ... */ <><svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Calculating...</>) : ( /* ... calculate icon ... */ <><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" /></svg>Calculate Projection</>)}
      </button>
    </form>
  );
};
