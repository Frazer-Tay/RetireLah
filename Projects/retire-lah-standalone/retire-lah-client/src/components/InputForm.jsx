  ...
  return (
    <form onSubmit={localFormSubmit} className="bg-brand-card-bg p-6 sm:p-8 rounded-xl shadow-card-lg space-y-6">
      ...
      {calcMode === CALC_MODES.TIME_TO_FIRE && (
        <div>
          <label htmlFor="plannedMonthlyInvestment" className="block text-sm font-medium text-brand-medium-text mb-1.5">Planned Monthly Investment (SGD)</label>
          <input type="number" name="plannedMonthlyInvestment" id="plannedMonthlyInvestment" value={formData.plannedMonthlyInvestment} onChange={handleChange} min="1" step="any" required className="py-2.5 px-3.5" placeholder="e.g., 1500"/>
        </div>
      )}
